...
import { feedbackApi } from '@/api/feedbackManagement'
import type {
  FeedbackItem,
  FeedbackDetail,
  FeedbackQueryParams,
...
export const useFeedbackManagementStore = defineStore('feedbackManagement', () => {
  ...
  const loadAvailableProcessors = async () => {
    try {
      ...
      if (response.data.code === 200) {
        availableProcessors.value = response.data.data
      } else {
        ElMessage.error(response.data.message || '获取处理人列表失败')
      }
    } catch (error) {
  ...
  const addInternalComment = async (id: number, data: CommentFormData) => {
    try {
      ...
      if (response.data.code === 200) {
        ...
        ElMessage.error(response.data.message || '评论添加失败')
      }
    } catch (error) {
  ...
  const updateFeedbackPriority = async (id: number, priority: FeedbackPriority) => {
    try {
      ...
      if (response.data.code === 200) {
        ...
        ElMessage.error(response.data.message || '优先级更新失败')
      }
    } catch (error) {
