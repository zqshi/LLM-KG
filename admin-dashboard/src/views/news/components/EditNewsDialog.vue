...
import type { NewsArticleForm, NewsArticle } from '@/types'
...
const loadArticleData = async () => {
  ...
  loading.value = true
  try {
    const article = await newsStore.fetchNewsDetail(props.articleId)
    if (article) {
      // 填充表单数据
      Object.assign(form, {
        title: article.title || '',
        ...
        effectiveTime: article.effectiveTime || '',
        expireTime: article.expireTime || '',
        visibilityScope: article.visibilityScope || 'ALL',
        category: article.category || '',
        tags: article.tags || []
      })
      
      // 设置可见范围类型
      visibilityType.value = Array.isArray(article.visibilityScope) ? 'DEPARTMENTS' : 'ALL'
...
const handleSubmit = async () => {
  if (!formRef.value || !props.articleId) return
  ...
  await formRef.value.validate(async (valid) => {
    ...
    try {
      const success = await newsStore.updateNews(props.articleId!, form)
      if (success) {
        visible.value = false
        emit('updated')
      }
