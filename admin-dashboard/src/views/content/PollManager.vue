  ...
  <div class="poll-manager">
    ...
    <el-card class="poll-list-card">
      ...
      <div v-else-if="pollList.length === 0" class="empty">
        <el-empty description="暂无投票帖">
          <el-button type="primary" @click="handleCreatePoll">创建投票帖</el-button>
        </el-empty>
      </div>
...
const loadCategories = async () => {
  try {
    const response = await PollAdminAPI.getCategories()
...
const loadOverviewStats = async () => {
  try {
    ...
    Object.assign(pollStats, response.data)
  } catch (error) {
