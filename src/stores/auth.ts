...
function generateDefaultMenus(): MenuNode[] {
  return [
    ...
    },
    {
      ...
      icon: 'Document',
      children: [
        { id: 21, name: '板块管理', path: '/content/categories', icon: 'FolderOpened' },
    ...
    },
    {
      ...
      icon: 'Setting',
      children: [
        { id: 81, name: '首页配置', path: '/operation/homepage', icon: 'House' },
        { id: 82, name: '推荐位管理', path: '/operation/recommendations', icon: 'Star' },
        { id: 83, name: '榜单管理', path: '/operation/rankings', icon: 'TrendCharts' },
        { id: 84, name: '数据看板', path: '/operation/dashboard', icon: 'DataBoard' }
      ]
    },
    {
      id: 8,
      ...
      icon: 'Tools',
      children: [
        { id: 91, name: '系统配置', path: '/system/settings', icon: 'Setting' },
        { id: 92, name: '审计日志', path: '/system/logs', icon: 'DocumentChecked' },
        { id: 93, name: '操作告警', path: '/system/alerts', icon: 'Bell' }
      ]
    }
  ]
}
    ...
    try {
      ...
      const isAdminUser =
        !!user.value && (
          ...
          (Array.isArray(user.value.roles) && user.value.roles.some(r => ['system_admin', 'super_admin', 'admin'].includes(r.code)))
        )
      ...
      if (storedToken && storedUser && storedPermissions) {
        ...
        if (storedMenus) {
          try {
            ...
            console.log('菜单数据详情:', menus.value.map(m => ({ name: m.name, path: m.path, childrenCount: m.children?.length || 0 })))
          } catch (e) {
          ...
          menus.value = generateDefaultMenus()
          console.log('重建后菜单数量:', menus.value.length)
          console.log('重建后菜单详情:', menus.value.map(m => ({ name: m.name, path: m.path })))
        }
  ...
  function checkPermission(permission: string): boolean {
    ...
    const isAdminUser =
      !!current && (
        ...
        (Array.isArray(current.roles) && current.roles.some(r => ['system_admin', 'super_admin', 'admin'].includes(r.code)))
      )
